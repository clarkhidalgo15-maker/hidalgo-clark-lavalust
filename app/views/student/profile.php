...
<body><div class="shell">
    ...
    <main><h1>Student profile</h1><div class="layout"><aside class="identity"><img class="avatar" src="<?= $escape(site_url('images/clark-profile.jpg')) ?>?v=ad5be59" alt="Photo of <?= $escape($name) ?>" onerror="this.style.display='none'; this.nextElementSibling.style.display='grid';"><div class="initials" style="display: none;">CDH</div><h2><?= $escape($name) ?></h2><span class="badge">BS INFORMATION TECHNOLOGY</span></aside><section class="details"><?php foreach ($details as $label => $value): ?><div class="item<?= $label === 'Address' ? ' wide' : '' ?>"><span class="label"><?= $escape($label) ?></span><span class="value"><?= $escape($value) ?></span></div><?php endforeach; ?></section></div></main>
</div></body>
</html>