...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; } body { margin: 0; background: var(--mist); color: var(--ink); font-family: Georgia, 'Times New Roman', serif; }
        ...
        header { display: flex; justify-content: space-between; align-items: center; padding: 24px 30px; border-bottom: 2px solid var(--ink); } .brand { display: flex; gap: 12px; align-items: center; font-weight: 800; } .mark { display: grid; place-items: center; width: 42px; height: 42px; overflow: hidden; background: var(--yellow); border: 2px solid var(--ink); border-radius: 50%; font-weight: 900; font-family: Arial, sans-serif; } .mark img { width: 100%; height: 100%; object-fit: cover; } nav { display: flex; gap: 10px; } nav a { color: var(--ink); text-decoration: none; border: 1px solid var(--ink); border-radius: 5px; padding: 10px 14px; font: 700 14px Arial, sans-serif; transition: .2s ease; } nav a:hover { background: var(--yellow); transform: translateY(-2px); }
...
<body><div class="shell">
    ...
    <main><h1>Student profile</h1><div class="layout"><aside class="identity"><img class="avatar" src="<?= $escape(base_url('images/clark-profile.jpg')) ?>?v=0bf894b" alt="Photo of <?= $escape($name) ?>" onerror="this.style.display='none'; this.nextElementSibling.style.display='grid';"><div class="initials" style="display: none;">CDH</div><h2><?= $escape($name) ?></h2><span class="badge">BS INFORMATION TECHNOLOGY</span></aside><section class="details"><?php foreach ($details as $label => $value): ?><div class="item<?= $label === 'Address' ? ' wide' : '' ?>"><span class="label"><?= $escape($label) ?></span><span class="value"><?= $escape($value) ?></span></div><?php endforeach; ?></section></div></main>
</div></body>
</html>